...
router.use(rateLimiting_1.aiRateLimiter);
router.post('/study-plan', async (req, res) => {
    try {
        ...
        const recentReports = await db_1.db.dailyReport.findMany({
            where: { userId },
            orderBy: { date: 'desc' },
            take: 7,
        });
        const todos = await db_1.db.todo.findMany({
        ...
        const plan = await (0, groqClient_1.generateStudyPlan)(user.examGoal, {
            daysLeft,
            avgStudyHours,
            avgCompletion,
...
router.post('/generate-tasks', async (req, res) => {
    try {
        ...
        const recentTodos = await db_1.db.todo.findMany({
            where: { userId },
        ...
        const tasks = await (0, groqClient_1.generateTasksWithGroq)(prompt, examGoal || user?.examGoal || 'exam', {
            recentTopics,
            daysUntilExam,
        });
        // Create todos in database
        const createdTodos = await Promise.all(tasks.map((task) => db_1.db.todo.create({
            data: {
                userId,
                title: task.title,
...
router.post('/exam-date', async (req, res) => {
    try {
        ...
        const { groq } = await Promise.resolve().then(() => __importStar(require('../lib/groqClient')));
