...
router.get('/conversations', auth_1.isAuthenticated, async (req, res) => {
    try {
        ...
        if (distinctUsers.length > 0) {
            distinctUsers[0].senders.forEach((id) => participantIds.add(id));
            distinctUsers[0].receivers.forEach((id) => participantIds.add(id));
        }
        // Remove self
...
router.get('/:userId', auth_1.isAuthenticated, async (req, res) => {
    try {
        const currentUserId = req.user.id;
        const { userId } = req.params;
        const messages = await db_1.db.directMessage.findMany({
            where: {
                $or: [
...
// Send a message
router.post('/:userId', rateLimiting_1.messageRateLimiter, auth_1.isAuthenticated, async (req, res) => {
    try {
        ...
        const newMessage = await db_1.db.directMessage.create({
            data: {
                senderId,
                receiverId: userId,
...
router.patch('/:userId/read', auth_1.isAuthenticated, async (req, res) => {
    try {
        const currentUserId = req.user.id;
        const { userId } = req.params;
        await db_1.db.directMessage.updateMany({
            where: {
                senderId: userId,
