...
const db_1 = require("../lib/db"); // MongoDB abstraction
const cache_1 = require("../lib/cache"); // Cache layer
...
router.get('/', async (req, res) => {
    try {
        ...
        cache_1.cache.set(cacheKey, todos, 120);
...
router.post('/', async (req, res) => {
    try {
        ...
        const todo = await db_1.db.todo.create({
            data: {
                ...req.body,
                userId,
                completed: false,
        ...
        // Invalidate cache
        cache_1.cache.delete(`todos:${userId}`);
        res.json(todo);
    }
    catch (error) {
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const existingTodo = await db_1.db.todo.findUnique({
        ...
        // Update todo
        const updatedTodo = await db_1.db.todo.update({
            where: { id: req.params.id },
            data: req.body,
        });
        ...
        if (req.body.completed && !existingTodo.completed) {
            ...
            cache_1.cache.invalidatePattern(`user:${userId}`);
        }
        // Invalidate caches
        cache_1.cache.delete(`todos:${userId}`);
