...
const generateMockHeatmapData = (): HeatmapData[] => {
  ...
  for (let i = 89; i >= 0; i--) {
    ...
    data.push({ date: date.toISOString().split('T')[0], level, count });
  }
...
export default function StudyHeatmap({ data }: StudyHeatmapProps) {
  const heatmapData = useMemo(() => data ?? generateMockHeatmapData(), [data]);
  const weeks = useMemo(() => {
                ...
                {week.map((day) => (
                  <Tooltip key={day.date}>
                    <TooltipTrigger asChild>
                      <div
                        className={`w-3 h-3 rounded-[3px] transition-transform duration-150 hover:scale-125 hover:z-20 cursor-crosshair ${getColorClass(day.level)}`}
                      />
                    </TooltipTrigger>
