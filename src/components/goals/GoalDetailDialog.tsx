...
function PlanRow({ item, index, count, targetDate, minDate, maxDate, onTargetDateChange, onToggle, onRename, onDelete, onMove }: {
  ...
  return <li className="flex flex-wrap items-start gap-1 rounded-xl border border-hairline p-2">
    <button type="button" aria-label={`${item.completed ? 'Reopen' : 'Mark'} ${item.title} plan complete`} onClick={onToggle} className={`press grid h-9 w-9 shrink-0 place-items-center rounded-lg border ${item.completed ? 'border-brand bg-brand text-on-accent' : 'border-ink/30 bg-ink/[0.04] hover:border-brand hover:bg-brand-subtle'}`}>{item.completed && <Check className="h-4 w-4" />}</button>
