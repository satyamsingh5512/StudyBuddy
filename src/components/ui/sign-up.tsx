...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize" }: AuthComponentProps) => {
  ...
  // Sign-in accepts any non-empty password; sign-up enforces the 8-char minimum.
  const passwordStepReady = mode === 'signup' ? isPasswordValid : password.length > 0;
  ...
  const handleGoogleAuth = () => {
    // Kick off the backend Google OAuth flow. On success the backend
    // redirects back with a #google_token=... hash which Providers/Auth handle.
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // Backend emails a 6-digit verification code. Move to the verify step.
        setModalStatus('closed');
        setAuthStep('verify');
    } catch (err: any) {
        setModalErrorMessage(err.message || 'Something went wrong.');
        setModalStatus('error');
    }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    if (!isOtpValid) {
        setModalErrorMessage('Please enter the 6-digit code from your email.');
        setModalStatus('error');
        return;
    }
  ...
  const handleSignIn = async () => {
    ...
    try {
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
            // Backend resends a code for unverified accounts — jump to verify step.
    ...
    <AnimatePresence>
        {modalStatus !== 'closed' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="glass-strong relative rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
                ...
                <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                     <AnimatePresence>
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                                ...
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="relative w-full">
                                        ...
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            ...
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", passwordStepReady ? "w-10 pr-1" : "w-0" )}><GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label={mode === 'signup' ? 'Submit password' : 'Sign in'} contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                        </div></div>
                                    </div>
                            ...
                            {authStep === 'email' && <BlurFade delay={0.25 * 6} inView className="w-full"><div className="flex justify-center pt-1"><button type="button" onClick={toggleMode} className="text-sm font-medium text-foreground/70 hover:text-foreground transition-colors">{mode === 'signup' ? 'Already have an account? Sign in' : 'New here? Create an account'}</button></div></BlurFade>}
                        </motion.div>}
                    </AnimatePresence>
                    <AnimatePresence>
                        ...
                        {authStep === 'verify' && <BlurFade key="verify-otp-field" className="w-full">
                            <div className="relative w-full">
                                <div className="glass-input-wrap w-[300px]"><div className="glass-input">
                                    ...
                                    <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isOtpValid ? "w-10 pr-1" : "w-0")}><GlassButton type="button" onClick={handleVerifyOtp} size="icon" aria-label="Verify code" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                </div></div>
                            </div>
                            <BlurFade inView delay={0.2}><button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground transition-colors"><ArrowLeft className="w-4 h-4" /> Go back</button></BlurFade>
                        </BlurFade>}
