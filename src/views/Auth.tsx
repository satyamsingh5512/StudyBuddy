...
export default function Auth() {
  const [authType, setAuthType] = useState<
    'signin' | 'signup' | 'verify-signup' | 'forgot-password' | 'verify-reset'
  >('signin');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authType === 'signup') {
        ...
        toast({
          title: 'Success!',
          description: data.message || 'Email verified. You are now logged in.',
        });
        setShowSuccess(true);
        setTimeout(() => {
        ...
        toast({
          ...
          description: data.message || 'Your password has been changed. Please sign in.',
        });
        setAuthType('signin');
        setPassword('');
        setConfirmPassword('');
  ...
  const fieldClass =
    'w-full h-12 rounded-2xl border border-border/60 bg-foreground/[0.035] text-foreground placeholder:text-muted-foreground/70 outline-none transition-all duration-200 focus:border-primary/45 focus:bg-foreground/[0.055] focus:ring-4 focus:ring-primary/15 dark:bg-white/[0.04] dark:focus:bg-white/[0.07]';
          ...
          <div className="relative hidden overflow-hidden md:block md:border-r md:border-white/10">
            ...
            <div className="absolute bottom-10 left-10 right-10 z-10 text-white">
              ...
              <p className="mt-2 text-sm leading-relaxed text-white/80">
                Join a community of students hitting their goals with AI-guided plans, focused
                sessions, and honest progress.
              </p>
          ...
          <div className="flex flex-col justify-center p-6 pt-16 sm:p-10 sm:pt-16 md:p-12">
            <div className="mb-8">
              ...
              <p className="text-muted-foreground text-sm">
                ...
                {(authType === 'verify-signup' ||
                  authType === 'forgot-password' ||
                  authType === 'verify-reset') && (
                  <button
                    type="button"
                    onClick={() => {
                      setAuthType('signin');
                ...
                {(authType === 'signin' || authType === 'signup') && (
                  <>
                    <div className="relative flex items-center py-3">
                      ...
                      <span className="px-4 text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground">
                        or
                      </span>
