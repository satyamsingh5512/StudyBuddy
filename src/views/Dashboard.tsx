...
const StudyHeatmap = dynamic(() => import('@/components/StudyHeatmap'), {
  loading: () => (
    <div
      ...
      aria-label="Loading consistency activity"
    />
  ),
});
const AnalyticsDashboard = dynamic(() => import('@/components/AnalyticsDashboard'), {
  loading: () => (
    <div className="h-64 rounded-2xl bg-muted/40 animate-pulse" aria-label="Loading analytics" />
  ),
});
...
const getTodoPriorityScore = (todo: Todo): number => {
  ...
  const difficultyScore =
    difficultyPriorityScore[todo.difficulty] ?? difficultyPriorityScore.medium;
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  {/* Difficulty badge */}
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold uppercase tracking-wide ${diffCfg.color}`}
                  >
                    {diffCfg.label}
                  </span>
                  <span
                  ...
                  {(() => {
                    const count =
                      typeof todo.rescheduledCount === 'number'
                        ? todo.rescheduledCount
                        : typeof todo.rescheduledCount === 'object' &&
                            todo.rescheduledCount &&
                            'increment' in todo.rescheduledCount &&
                            typeof (todo.rescheduledCount as any).increment === 'number'
                          ? (todo.rescheduledCount as any).increment
                          : 0;
                    return (
                      count > 0 && (
              ...
              <div className="flex items-center gap-0.5 flex-shrink-0">
                <button
                  ...
                  className="opacity-60 group-hover:opacity-100 focus-visible:opacity-100 transition-opacity p-1.5 hover:bg-primary/10 rounded-lg"
                ...
                <button
                  ...
                  className="opacity-60 group-hover:opacity-100 focus-visible:opacity-100 transition-opacity p-1.5 hover:bg-destructive/10 rounded-lg"
...
export default function Dashboard() {
  ...
  } = useDailyEfficiency(1);
  const dailyEfficiency = useMemo(
    () => dailyEfficiencyData as DailyEfficiency | null,
    [dailyEfficiencyData]
  );
  ...
  const toggleTodo = useCallback(
    async (id: string, completed: boolean) => {
      ...
      setDndTodos((prev) =>
        prev.map((t) =>
          t.id === id
            ? { ...t, completed: !completed, isOverdue: !completed ? false : t.isOverdue }
            : t
        )
      );
      ...
      try {
        ...
        // Revert on error
        setDndTodos((prev) =>
          prev.map((t) => (t.id === id ? { ...t, completed, isOverdue: t.isOverdue } : t))
        );
    ...
    async (id: string) => {
      try {
        ...
        toast({
          ...
          description: `Complete it to earn points!${pointsMsg}`,
        });
      } catch (error) {
  ...
  const rescheduleAllOverdue = useCallback(async () => {
  ...
  const deleteAllForToday = useCallback(async () => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
  ...
  const editTodo = useCallback(
    async (id: string, updates: Partial<Todo>) => {
      ...
      setDndTodos((prev) => prev.map((t) => (t.id === id ? { ...t, ...updates } : t)));
  ...
  const handleReschedule = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Scheduled for ${new Date(rescheduleDate).toLocaleDateString()}${pointsMsg}`,
      });
    } catch (error) {
      console.error('Error rescheduling todo:', error);
  ...
  const overdueCount = useMemo(
    () => todos.filter((t) => t.isOverdue && !t.completed).length,
    [todos]
  );
      ...
      >
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              {user?.examDate
                ...
                : user?.examGoal
                  ? `Preparing for ${user.examGoal}`
                  : 'Build a study plan for your next goal'}
            </p>
          </div>
        ...
        >
          ...
          <MotionCard
            ...
            className="group relative min-w-0 overflow-hidden rounded-xl bg-background/40 sm:min-w-[180px] xl:flex-shrink-0"
          >
          ...
          {overdueCount > 0 && (
            <MotionCard
              variants={item}
              whileHover={hoverLift}
              className="group relative min-w-0 overflow-hidden rounded-xl bg-background/40 sm:min-w-[180px] xl:flex-shrink-0 line-through-animation"
            >
              <CardContent className="relative flex h-full flex-col justify-between p-4 sm:p-6">
          ...
          <MotionCard
            variants={item}
            whileHover={hoverLift}
            className="group relative min-w-0 overflow-hidden rounded-xl bg-background/40 sm:min-w-[220px] xl:flex-shrink-0"
          >
            <CardContent className="relative flex h-full flex-col justify-between p-4 sm:p-6">
              <div className="absolute top-0 right-0 w-24 h-24 bg-primary/5 rounded-full blur-2xl -translate-y-8 translate-x-8 group-hover:scale-150 transition-transform duration-700"></div>
              <div className="relative z-10">
                ...
                <div className="mt-auto">
                  ...
                  {!efficiencyLoading && (
                    <p className="text-[11px] text-muted-foreground mt-1">
                      {dailyEfficiency?.abandonedTimerStarts || 0} abandoned starts • -
                      {(dailyEfficiency?.strictPenaltyPoints || 0).toFixed(1)} penalty
                    </p>
                  )}
                </div>
              </div>
          ...
          <SolidCard className="border-orange-200 dark:border-orange-800 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-950/20 dark:to-amber-950/20">
            ...
            <SolidCardContent>
              <p className="text-sm text-orange-800 dark:text-orange-200 mb-4">
                You have administrator privileges. Access the admin dashboard to manage users and
              ...
              <CardContent className="space-y-4 pt-4">
                <div className="flex gap-2 relative">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    title="Add task"
                    className="absolute right-0 hover:bg-transparent hover:text-primary"
                  >
                ...
                <div className="w-40">
                  <Select
                    ...
                    onValueChange={(value) =>
                      setNewTodoDifficulty(value as 'easy' | 'medium' | 'hard')
                    }
                  >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                                        <p className="text-xs text-muted-foreground">
                                          {activeDragTodo.subject}
                                        </p>
                                        <span
                                          className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold uppercase tracking-wide ${(difficultyConfig[activeDragTodo.difficulty] ?? difficultyConfig.medium).color}`}
                                        >
                                          {
                                            (
                                              difficultyConfig[activeDragTodo.difficulty] ??
                                        ...
                                        <span
                                          className={`text-xs px-1.5 py-0.5 rounded-full flex items-center gap-1 ${
                                            activeDragTodo.isOverdue && !activeDragTodo.completed
                                              ? 'bg-rose-100 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400'
                                              : 'bg-muted text-muted-foreground'
                                          }`}
                                        >
                      ...
                      <AnimatePresence>
                        {todos.length === 0 && (
                          ...
                          >
                            ...
                            <p className="text-xs text-muted-foreground">
                              Add a task above to start earning points
                            </p>
                          </motion.div>
                        )}
                      </AnimatePresence>
        ...
        <GlassModal
          ...
          onOpenChange={(open) =>
            setRescheduleModal({ open, todoId: open ? rescheduleModal.todoId : null })
          }
